...
#ifdef KNX_TUNNELING
    /* For a pure IP device there is only one data link layer.
     * The cEMI server uses _dataLinkLayerPrimary to send M_Prop responses
     * and tunnelling indications back to ETS.  Without this assignment it
     * remains nullptr, causing a null-pointer crash the first time ETS sends
     * a DeviceConfigurationRequest (M_PropRead). */
    _cemiServer.dataLinkLayerPrimary(_dlLayer);
#endif
...
void Bau57B0::syncCemiClientAddress()
{
#ifdef USE_CEMI_SERVER
