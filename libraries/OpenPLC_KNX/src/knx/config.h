#pragma once
    ...
    /* OpenPLC extension: IP+TP dual application device (MASK 0x5780).
     * Has group objects like a normal device AND communicates on both media. */
    ...
    /* Enable KNXnet/IP Tunneling when IP transport is active.
     * ETS needs DescriptionRequest/ConnectRequest support (KNXnet/IP Core
     * sections 7.5-7.6) to discover and program IP-capable devices.
     * ETS 5/6 opens TWO simultaneous connections per session:
     *   - one TUNNEL_CONNECTION (type 0x04) for data/monitoring
     *   - one DEVICE_MGMT_CONNECTION (type 0x03) for programming/configuration
     * Set KNX_TUNNELING to at least 2 so both can be accepted. */
    #if defined(USE_IP) && !defined(KNX_TUNNELING)
    #  define KNX_TUNNELING 2
    #endif
