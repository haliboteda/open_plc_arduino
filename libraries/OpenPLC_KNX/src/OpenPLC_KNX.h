 ...
 *
 * MASK_VERSION is set in stm32h743_openplc_platform.h (default 0x57B0) or
 * may be overridden per-sketch via a -D compiler flag or board variant.
 *
 *   0x07B0  KNX TP device (TP-only, application group objects)
 *   0x57B0  KNXnet/IP device (IP-only, application group objects)
 *   0x091A  IP/TP1 coupler (routes between IP and TP, no local GOs)
 *
 * For 0x07B0 and 0x57B0, KNX.getGroupObject(n) is available.
...
#else
  #error "OpenPLC_KNX: unsupported MASK_VERSION. Use 0x07B0 (TP device), 0x57B0 (IP device), or 0x091A (IP/TP coupler)."
#endif
