 ...
 * IAP_boot_handoff.c
 *
 * See IAP_boot_handoff.h. Mirrors open_plc_cube_ide/IAPServer/IAP_boot_handoff.c -- the
 * two copies differ only in the include block below, same arrangement as
 * iap_auth.c / iap_keyderive.c.
 *
 * boot_handoff_take() is never called from the application image (only the
 * bootloader consumes requests); it is kept here so both copies stay identical
 * and reviewable side by side.
 *
 * This lives in the core rather than in libraries/OpenPLC_IAP alongside
 * iap_auth.c, because the CDC magic-baud request site is core code
 * (cores/arduino/stm32/usb/cdc/usbd_cdc_if.c) and the Arduino build does not put
 * a library's src/ directory on the include path of core sources. Core headers
 * are visible to libraries, so putting it here lets both request sites -- the
 * CDC one in the core and the UDP one in OpenPLC_IAP -- share one copy.
 */
 ...
 * has configured its caches, and it needs no MPU region. A user's PLC sketch is
 * free to enable the D-cache without breaking the handoff.
 */
